...
const {formatDate} = require('./jalali.js');
...
function startServer() {
    ...
    // Delete Didar
    app.get(['/deleteDidar/:id'], async(req, res) => {
        try {
    ...
    app.post(['/users/register'], async (req, res) => {
        try {
            if(hostCfg.admin){
                ...
                return res.status(200).json({message: "عضویت شما با موفقیت انجام شد"});
            }
            else{
                return res.status(400).json({message:"مشکلی هست"});
            }
    ...
    app.post(['/login'], async (req, res) => {
        ...
        try {
            ...
            if(isMatch){
                ...
                res.status(200).json({ message: {token: token, username: username, fullname: fullname, user_type: user_type} });
            } else {
